...
namespace Microsoft.Automata
{
    ...
    public partial class CountingAutomaton<S> : Automaton<Tuple<Maybe<S>, Sequence<CounterOperation>>>
    {
        ...
        {
            string s;
            __hideDerivativesInViewer = true;
...
*/
        ...
        /// <returns></returns>
        public CsAutomatonOpt<S> CompileOpt(int _mode = 0, bool graphs = false, bool merge = false)
        {
                ...
                foreach (var move1 in movesOut)
                {
                    ...
                    for (var r = 0; r < counterMinterms.Length; r++)
                    {
                        ...
                        if (graphs)
                        {
                           ...
                           if(!merge)
                                moves.Add(Move<CsLabel<S>>.Create(sourceState, tgt, CsLabel<S>.MkTransitionLabel(pred, rule, productAlgebra, newUpdates, ((CABA<S>)this.Algebra).builder.solver.PrettyPrint)));
                            if (merge)
                            { 
        ...
        public string PrettyPrint(Tuple<Maybe<S>, Sequence<CounterOperation>> t)
        {
            ...
            if (t.Item1.IsSomething)
            {
                if (t.Item2.Length > 0)
                {
                    ...
                    if (t.Item2.Length == 1)
                    {
                        var str = builder.solver.PrettyPrint(t.Item1.Element);
                        str = str.Replace(".", " \u2022 ");
                        var update = t.Item2[0].ToString();
                        if (update.Length != 0)
                            str += prod + update;
                        return str;
