...
namespace Microsoft.Automata
{
    ...
    public partial class CountingAutomaton<S> : Automaton<Tuple<Maybe<S>, Sequence<CounterOperation>>>
    {
            ...
            if (countingStates != null)
            {
                if (IsFinalState(state) && IsCountingState(state))
                {
                    ...
                    for (int i = 0; i < f.Length; i++)
                    {
                        ...
                        var op = f[i];
                        s += String.Format("c{0}9", SpecialCharacters.GEQ);//op.ToStringPredicate();
                    }
        ...
        private ValueTuple<CsUpdateSeq, int, bool, HashSet<int>, HashSet<int>, bool> GetTargetInfo(Move<Tuple<Maybe<S>, Sequence<CounterOperation>>>[] moves, S minterm, CsConditionSeq rule, PowerSetStateBuilder dfaStateBuilder)
        {
            ...
            return new ValueTuple<CsUpdateSeq, int, bool, HashSet<int>, HashSet<int>, bool>(CsUpdateSeq.Mk(updates), dfaStateBuilder.MakePowerSetState(targetStates), final, updatedCounters, finalCounters, nonCond);
        }
...
*/
        ...
        public CsAutomatonOpt<S> CompileOpt(int _mode = 0, bool graphs = false)
        {
            ...
            var transTable = new Dictionary<int, List<ValueTuple<CsConditionSeq, ValueTuple<CsUpdate[], int>>>[]>();
            ...
            var minterms = new int[partitions.Count()];
            ...
            if (this.IsFinalState(this.InitialState))
            {
                ...
                if(IsCountingState(this.InitialState))
                    finalCounters[1] = new HashSet<int> { this.GetCounter(this.InitialState).CounterId };
                else
            ...
            while (stack.Count > 0)
            {
                ...
                transTable[sourceState] = new List<ValueTuple<CsConditionSeq, ValueTuple<CsUpdate[], int>>>[minterms.Length];
                ...
                foreach (var move1 in movesOut)
                {
                    ...
                    transTable[sourceState][symbol] = new List<ValueTuple<CsConditionSeq, ValueTuple<CsUpdate[], int>>>();
                    for (var r = 0; r < counterMinterms.Length; r++)
                    {
                        ...
                        if (graphs)
                        {
                            CsUpdateSeq updateSequence = null;
                            if (cntCount != 0)
                            {
                                updateSequence = CsUpdateSeq.Mk(updates);
                            }
                            moves.Add(Move<CsLabel<S>>.Create(sourceState, tgt, CsLabel<S>.MkTransitionLabel(minterm, rule, productAlgebra, updateSequence, ((CABA<S>)this.Algebra).builder.solver.PrettyPrint)));
                        }
                        countMoves++;
                        transTable[sourceState][symbol].Add(new ValueTuple<CsConditionSeq, ValueTuple<CsUpdate[], int>>(rule, new ValueTuple<CsUpdate[], int>(updates, tgt)));
                    }  
                }
            }
            var autDet = Automaton<CsLabel<S>>.Create(null, 1, finalStateDCA, moves);
           // autDet.ShowGraph("autDet");
            CsAutomatonOpt<S> autOpt = new CsAutomatonOpt<S>(CABAsolver, autDet, countMoves, dfaStateBuilder, transTable, countStates, finalStateDCA.ToArray(), counters, this.cntCount, precomputed, minterms, mapStates, finalCounters, updatedCounters, usedCounters);
            return autOpt;
        }       
      ...
      /*  private IEnumerable<CsConditionSeq> GenerateCounterMinterms(ConsList<int> list, S a, int _mode = 0)
        ...
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private IEnumerable<CsConditionSeq> GenerateMinterms(ConsList<int> list, List<int> uc, Move<Tuple<Maybe<S>, Sequence<CounterOperation>>>[] moves, int _mode, CsConditionSeq trueSeq)
        {
            if (list == null)
            ...
            else
            {
                ...
                var i = GetCounter(list.First).CounterId;
                foreach (var seq in GenerateMinterms(list.Rest, uc, moves, _mode, trueSeq))
                {
                    // this can be just in the case that there is one exit or one incr for all counting states not just first in a list
                     if (incr_exists || exit_exists)
                     {
        ...
        bool __useInternalNamesInViewer = false;
        public void ShowGraph(string name = "CountingAutomaton", bool hideDerivatives = false, bool internalNames = false)
        {
            __hideDerivativesInViewer = hideDerivatives;
            __useInternalNamesInViewer = internalNames;
            base.ShowGraph(name);
        }

        public void SaveGraph(string name = "CountingAutomaton", bool hideDerivatives = false, bool internalNames = false)
        {
            __hideDerivativesInViewer = hideDerivatives;
            __useInternalNamesInViewer = internalNames;
            base.SaveGraph(name);
    ...
    internal class CABA<S> : IBooleanAlgebra<Tuple<Maybe<S>, Sequence<CounterOperation>>>, IPrettyPrinter<Tuple<Maybe<S>, Sequence<CounterOperation>>>
    {
        ...
        public string PrettyPrint(Tuple<Maybe<S>, Sequence<CounterOperation>> t)
        {
            if (t.Item1.IsSomething)
            {
                if (t.Item2.Length > 0)
                {
                    char prod = SpecialCharacters.AND;
                    if (t.Item2.Length == 1)
                    {
                        //if (t.Item1.Element.Equals(builder.solver.True))
                        //    return t.Item2[0].ToString();
                        //else
            ...
            else
            {
                ...
                for (int i = 0; i < t.Item2.Length; i++)
                {
                    ...
                    if (t.Item2[i].Counter.LowerBound > 0)
                    {
                        if (s != "")
                            s += SpecialCharacters.AND;
                        //do not write index 0
                        var c = (t.Item2[i].Counter.CounterId == 0 ? "c" : SpecialCharacters.c(t.Item2[i].Counter.CounterId));
                        s += string.Format("{0}{1}{2}", c, SpecialCharacters.GEQ, t.Item2[i].Counter.LowerBound);
                    }
                }
