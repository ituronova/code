...
namespace Microsoft.Automata
{
    ...
    public class CounterOperation  
    {
        ...
        public string CounterIndex
        {
            get
            {
                if (Counter.CounterId == 0)
                    ...
                    return SpecialCharacters.ToSubscript(Counter.CounterId);
            }
        }
        ...
        string UpperBoundCheck
        {
            get
            {
                if (Counter.UpperBound < int.MaxValue)
                    return string.Format("c<{1}", SpecialCharacters.c(Counter.CounterId), Counter.UpperBound);
        ...
        //        case CounterOp.EXIT:
        //            //return LowerBoundCheck;
        //            return string.Format("{2}{3}{0}{4}0", SpecialCharacters.c(Counter.CounterId), SpecialCharacters.CIRCLE, LowerBoundCheck, '/', SpecialCharacters.ASSIGN);
        //        case CounterOp.EXIT_SET1:
        //            return string.Format("{2}{3}{0}{4}1", SpecialCharacters.c(Counter.CounterId), SpecialCharacters.CIRCLE, LowerBoundCheck, '/', SpecialCharacters.ASSIGN);
        //        case CounterOp.INCR:
        ...
        //            return string.Format("{2}{3}{0}++", SpecialCharacters.c(Counter.CounterId), SpecialCharacters.ASSIGN, UpperBoundCheck, '/');
        //        default:
        //             throw new NotImplementedException(elems.Item2.ToString());
        //    }
        //}
        ...
        public string ToStringPredicate()
        {
            switch (elems.Item2)
            {
                ...
                case CounterOp.EXIT:
                case CounterOp.EXIT_SET1:
                    return string.Format(SpecialCharacters.XI_LOWERCASE + "(" + CounterName + ")");
                case CounterOp.INCR:
                    return string.Format(SpecialCharacters.IOTA_LOWERCASE + "(" + CounterName + ")");
                default:
                    {
                        throw new NotImplementedException(elems.Item2.ToString());
                    }
        ...
        public override string ToString()
        {
            ...
            {
                case CounterOp.SET0:
                    return "";//string.Format("SET0({0})", CounterName);
                case CounterOp.SET1:
                    return string.Format("/c{0}1", SpecialCharacters.ASSIGN); //string.Format("SET1({0})", CounterName);
                case CounterOp.EXIT_SET1:
                    return string.Format("{1}/c{0}1", SpecialCharacters.ASSIGN, LowerBoundCheck, SpecialCharacters.AND);//string.Format("EXIT1({0})", CounterName);
                case CounterOp.EXIT:
                    return string.Format("EXIT(c)", CounterName);//string.Format("EXIT({0})", CounterName);
                case CounterOp.INCR:
                    return string.Format("{1}c++", CounterName, UpperBoundCheck, SpecialCharacters.AND);
                default:
                    throw new NotImplementedException(elems.Item2.ToString());
            }
        }
