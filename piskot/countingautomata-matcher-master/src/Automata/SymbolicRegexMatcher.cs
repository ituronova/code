...
namespace Microsoft.Automata
{
    ...
    public class SymbolicRegex<S> : RegexMatcher
    {
        ...
        /// unspecified and taken to be input.Length-1</param>
        /// </summary>
        public bool IsMatchCA(string input, CsAutomatonOpt<S> aut,  bool caching = false, CSMatcher matcher = null, int startat = 0)
        {
            int i;
            if (this.A_prefix != string.Empty)
            {
                i = FindFinalStatePositionCAOpt(input, aut, input.Length, startat);
            }
            else
            {   // main function
                i = FindFinalStatePositionCA(input, aut, input.Length, startat, caching, matcher);
            }
            if (i == input.Length)
            {
                //the search for final state exceeded the input, so final state was not found
        ...
        /// <param name="i_q0">last position the initial state of A1 was visited</param>
        /// <param name="k">input length or bounded input length</param>
        private int FindFinalStatePositionCA(string input, CsAutomatonOpt<S> aut, int k, int i, bool caching = false,  CSMatcher matcher = null)
        {
            ...
            {   // initialize counting sets
                ...
                {
                    aut.cs[j].Clear(aut.counters[j]);     // initialization to 0   
                }
            }
            // initial state
            ...
            i = IndexOfStartset(input, i);
            if (i != 0 && this.A_startAnchor)    // check if the regex start at the beggining of the line
                return k;
            if (i == -1)
            {
                return k;
            ...
            while (i < k)
            {
             ...
             /*   aut.dot += aut.currentState + "[label=\"";
                for (int j = 0; j < aut.cntCount; j++)
                {
                    aut.dot += aut.cs[j].ToString();
                }
                aut.dot += "\"]\n";
                aut.dot += aut.currentState + "->";*/
               ...
               // aut.sizeCS.Add(aut.cs[0].CountElements);
               /* aut.dot += aut.currentState + "[label=\""+c+"\"]\n";
                aut.dot += aut.currentState + "[label=\"";
                for(int j = 0; j < aut.cntCount; j++)
                {
                    aut.dot += aut.cs[j].ToString();
                ...
                {
                    if(i == k-1 || !this.A_endAnchor)   // check the end of line if there is an end anchor
                        break;
                }                     
                i += 1;
                /* if (caching)
         ...
         /*   using (System.IO.StreamWriter file =
            new System.IO.StreamWriter(@"C:\Users\42073\Downloads\countingautomata-ce301c6e3513cbd6d489a7e80790690d197beb1d\graph.dot"))
            {
                file.WriteLine(aut.dot + "}");
            }*/
            return i;
        }
        ...
        private int FindFinalStatePositionCAOpt(string input, CsAutomatonOpt<S> aut, int k, int i)
        {
           ...
           {
               aut.currentState = prefix.Length+1;
               //skip the prefix
               i = i + prefix.Length;
               if (i == k)
               {
