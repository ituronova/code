...
namespace Microsoft.Automata
{
            ...
            else
            {
                ...
                else
                {
                    foreach (var path in EnumerateCases((BDD<T>)bdd.Zero))
                    {
                        ...
                        var v = path.Item1[i];
                        /*if(path.Item1 == alg.TrueCsConditionSeq)
                            yield return new Tuple<CsConditionSeq, T>(alg.TrueCsConditionSeq, path.Item2);
                        else
                        { */
                          /*  if (is_canexit_bit)
    ...
    public class CsAlgebra<T> : IBooleanAlgebra<CsPred<T>>, IPrettyPrinter<CsPred<T>>
    {
        ...
        public bool GetCondition(int c, BDD<T> t)
        {
            return true;
        }
        ...
        public bool IsFalse(CsConditionSeq seq)
        {
            return (seq == FalseCsConditionSeq);
        }
