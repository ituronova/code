...
namespace Microsoft.Automata
{
    ...
    {
        int[][] transitionTable;
        ValueTuple<CsUpdate[], int>[][] transInfoTable;
        Dictionary<int, ValueTuple<CsUpdate[], int>>[] tInfoTable;
        ...
        public CsAutomatonOpt(ICharAlgebra<S> algebra, Automaton<CsLabel<S>> aut, int countMoves, PowerSetStateBuilder stateBuilder, Dictionary<int, List<ValueTuple<CsConditionSeq, ValueTuple<CsUpdate[], int>>>[]> transTable, int countStates, int[] finalStateDCA, ICounter[] counters, int countCounters, int[] precomputed, int[] minterms, Dictionary<int, int> mapping, Dictionary<int, HashSet<int>> finalCounters, Dictionary<int, HashSet<int>[]> updatedCounters, Dictionary<int, HashSet<int>> usedCounters) : base(aut)
        {
            this.stateBuilder = stateBuilder;
            this.inputAlgebra = algebra;
            this.cntStates = countStates;
            this.sizeCS = new List<int>();
            this.finalSet = new bool[this.cntStates + 1];
            for (int i = 0; i < finalStateDCA.Length; i++)
            {
                this.finalSet[finalStateDCA[i]] = true;
            }
            ...
            this.transInfoTable = new ValueTuple<CsUpdate[], int>[countMoves+1][];
            this.tInfoTable = new Dictionary<int, ValueTuple<CsUpdate[], int>>[countMoves + 1];
            ...
            for (int i = 1; i <= this.cntStates; i++)
            {
               ...
               // this.usedCounters[i] = new int[minterms.Length];
                if (this.finalSet[i])
                    this.finalCounters[i] = finalCounters[i].ToArray();
                
                this.usedCounters[i] = usedCounters[i].ToArray();
                ...
                for (int j = 0; j < minterms.Length; j++)
                {
                    ...
                    ValueTuple<CsUpdate[], int>[] condTable = null;
                  //  Dictionary<int, ValueTuple<CsUpdate[], int>> cTable = null;
                  //  if(this.usedCounters[i].Length < 14)
                         condTable = new ValueTuple<CsUpdate[], int>[(long)Math.Pow(2, this.usedCounters[i].Length * 2)];
                        //condTable = new ValueTuple<CsUpdate[], int>[(long)Math.Pow(2, 13 * 2)];
                    //  condTable = new ValueTuple<CsUpdate[], int>[(long)Math.Pow(2, cntCount)];                  
                    //else
                      //  cTable = new Dictionary<int, ValueTuple<CsUpdate[], int>>();
                    ...
                    for (int m = 0; m < moves.Count(); m++)
                    {
                        ...
                        {
                            int condIndex = move.Item1.CountIndex(this.usedCounters[i]);
                         //  if (this.usedCounters[i].Length < 14)
                                condTable[condIndex] = move.Item2;
                           // else
                             //   cTable[condIndex] = move.Item2;
                        }
                    }
                    if (this.updatedCounters[i][minterm].Length != 0)
                    {
            ...
            */
            ...
            if (state_counters != null)
            {
                ...
                foreach (var c in state_counters_list)
                {
                    ...
                    if (finalSet[c])
                    {
                        s += SpecialCharacters.XI_LOWERCASE + "(" + SpecialCharacters.c(c) + ")";
                        s += SpecialCharacters.CHECKMARK;
                    }
                }
                if (IsFinalState(state))
                {
                    s += "\nF: " + SpecialCharacters.TOP;
                }
            }
        ...
        public override string DescribeStartLabel()
        {
            ...
            if (initcounters.Count != 0)
            {
                ...
                return string.Format("c{0}{{0}}", SpecialCharacters.ASSIGN);
            }
            else
                return "";
        }
        ...
        public List<int> GetCountersOfState(int state)
        {
            ...
            for (int i = 0; i < updatedCounters[state].Count(); i++)
            {
        ...
        public int Delta(int symbol)
        {
            ...
            else if (target < 0)    // key to transition table
            {
                // count index to transition table
                 ...
                 {                
                     index = index | cs[ac[c]].flag << c * 2;
                 }
                var chosenTran = new ValueTuple<CsUpdate[], int>();
                    chosenTran = this.transInfoTable[-target][index];
                if (chosenTran.Item2 == 0)
                    return -1;

                for (var a = 0; a < updatedCounters[currentState][symbol].Length; a++)
                {
                    ...
                    switch (chosenTran.Item1[i])
                    {
                        ...
                        case CsUpdate.COPY:
                            break;
                        default:
                            cs[i].Clear(this.counters[i]);
                            break;
                    }
                    // count flag for counter
                    var max = cs[i].Max;
                    if (max >= counters[i].LowerBound)
                    {
                        // NOTICE that I use CANLOOP insted of HIGH to have smaller array for successors
                        if (max >= counters[i].UpperBound && cs[i].IsSingleton)
                            cs[i].flag = (int)CsCondition.CANLOOP;
                        else
...
#region replace set with incr if possible
    ...
    public class CsLabel<S>
    {
        ...
        CsLabel(bool isFinalCondition, S symbol, CsConditionSeq guards, CsAlgebra<S> alg, CsUpdateSeq updates, Func<S, string> inputToString)
        {
        ...
        public static CsLabel<S> MkFinalCondition(S symbol, CsConditionSeq guards, CsAlgebra<S> alg, Func<S, string> inputToString = null)
        {
            return new CsLabel<S>(true, symbol, guards, alg, null, inputToString);
        }

        public static CsLabel<S> MkTransitionLabel(S symbol, CsConditionSeq guards, CsAlgebra<S> alg, CsUpdateSeq updates, Func<S, string> inputToString = null)
        {
            return new CsLabel<S>(false, symbol, guards, alg, updates, inputToString);
        }
        ...
        internal string ToString(bool debugmode)
        {
            ...
            var pp = LeafAlg.LeafAlgebra as ICharAlgebra<S>;
            if (cond != "")
                cond += SpecialCharacters.OR + "\n";
            var s = this.Symbol;
            //cond += s.ToString();
            cond += (pp != null ? pp.PrettyPrint(s) : s.ToString());
            var g = this.Guard;
            if(g != null)
            {
                var countercond = (debugmode ? g.ToString() : g.ToString());// : g.ToString<S>(LeafAlg));
    ...
    public class CsUpdateSeq
    {
        ...
        public override string ToString()
        {
            ...
            for (int i = 0; i < Length; i++)
                ...
                {
                    if (this[i] == CsUpdate.INCR0)
                    {
                        s += string.Format("INCR({0}), RST({0})", SpecialCharacters.c(i));

                    }

                    else if (this[i] == CsUpdate.ADD0)
                    {
                        //s += "";
                        s += string.Format("RST({0})", SpecialCharacters.c(i));
                    }
                    /*  else if (this[i] == CsUpdate.INCR)
                      {
                          //s += "RST(c)";
                          s += string.Format("c++", SpecialCharacters.ASSIGN);
                      }*/
                    else
    ...
    public class CsConditionSeq
    {
        ...
        private string ExctractPredicate(int i)
        {
            string s = "";
            switch (this[i])
            {
                case CsCondition.CANEXIT:
                    s= SpecialCharacters.XI_LOWERCASE + "(" + SpecialCharacters.c(i) + ")";
                    return s;
                case CsCondition.CANLOOP:
                    s = SpecialCharacters.IOTA_LOWERCASE + "(" + SpecialCharacters.c(i) + ")";
                    return s;
                case CsCondition.CANNOTEXIT:
                case CsCondition.LOW:
                    s=SpecialCharacters.NOT.ToString() + SpecialCharacters.XI_LOWERCASE + "(" + SpecialCharacters.c(i) + ")";
                    return s;
                case CsCondition.CANNOTLOOP:
                case CsCondition.HIGH:
                    s=SpecialCharacters.NOT.ToString() + SpecialCharacters.IOTA_LOWERCASE + "(" + SpecialCharacters.c(i) + ")";
                    return s;
                case CsCondition.MIDDLE:
                    s = SpecialCharacters.XI_LOWERCASE + "(" + SpecialCharacters.c(i) + ")" + 
                        SpecialCharacters.AND.ToString() + SpecialCharacters.IOTA_LOWERCASE + "(" + SpecialCharacters.c(i) + ")";
                    return s;
                default:
                    s = this[i].ToString() + "(" + SpecialCharacters.c(i) + ")";
                    return s;
            }
        }
        ...
        public int CountIndex(int[] activeCounters)
        {
            ...
            for (int c = 0; c < activeCounters.Length; c++)
            {
                ...
                /*  ulong bitmask = ((ulong)1) << i;
                  if ((Low & bitmask) != 0)
                      res = res | (int)(CsCondition.LOW) << i * 4;
                  if ((Middle & bitmask) != 0)
                      res = res | (int)(CsCondition.MIDDLE) << i * 4;
                  if ((High & bitmask) != 0)
                      res = res | (int)(CsCondition.HIGH) << i * 4;*/
            }
            return (int)res;
        }
